...
import { Home, Gamepad2, Swords, Trophy, ShoppingBag, User, LogOut, Wallet, History, BookOpen, ChevronLeft, ChevronRight, MessageCircle } from 'lucide-react';
...
export default function Sidebar({ collapsed, onToggle }) {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-full bg-[hsl(230,25%,7%)] border-r border-border z-40 transition-all duration-300 flex flex-col ${collapsed ? 'w-16' : 'w-56'}`}>
      {/* Logo */}
      ...
      {/* Wallet */}
      {!collapsed && (
        <Link to="/shop" className="mx-3 mt-3 p-3 rounded-xl bg-gradient-to-r from-[hsl(187,92%,55%,0.08)] to-[hsl(270,60%,55%,0.08)] border border-[hsl(187,92%,55%,0.15)] hover:border-[hsl(187,92%,55%,0.3)] transition-colors">
          <div className="flex items-center gap-2">
            <Wallet className="w-4 h-4 text-yellow-400" />
            <div>
              <p className="text-[9px] text-muted-foreground uppercase tracking-wider">Portefeuille</p>
              <p className="text-sm font-bold text-yellow-400 font-mono">{wallet.balance.toLocaleString()} <span className="text-[10px] font-normal">jetons</span></p>
            </div>
          </div>
        </Link>
      )}
      ...
      <nav className="flex-1 p-2 mt-2 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map(item => {
          ...
          return (
            ...
            >
              <item.icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-[hsl(187,92%,55%)]' : ''}`} />
